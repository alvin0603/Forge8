...
class TraceWorkerTests(unittest.TestCase):
    ...
    def test_bootstrap_static_identity_filter_and_call_only_finally_contract(self):
        ...
        self.assertNotIn("f_locals", ast.unparse(callback))
        watch_call = next(node for node in callback.body if isinstance(node, ast.If)
                          and ast.unparse(node.test) == "watch_names")
        self.assertEqual(ast.unparse(watch_call.body[0]), "watch_event(frame, event)")
        ...
        watch_branch = next(node for node in call_branch.body if isinstance(node, ast.If)
                            and any(isinstance(child, ast.FunctionDef) and child.name == "watch_event"
                                    for child in node.body))
        self.assertEqual(ast.unparse(watch_branch.test), "watch_names")
        watcher = next(node for node in watch_branch.body if isinstance(node, ast.FunctionDef))
        local_reads = [node for node in ast.walk(tree) if isinstance(node, ast.Attribute) and node.attr == "f_locals"]
        self.assertEqual(len(local_reads), 1)
        self.assertLess(watcher.lineno, local_reads[0].lineno)
        self.assertLessEqual(local_reads[0].lineno, watcher.end_lineno)
        ...
        invoke = guarded.body[1].value
        self.assertIsInstance(invoke, ast.Call)
        self.assertIsInstance(invoke.func, ast.IfExp)
        self.assertEqual(ast.unparse(invoke.func.test), "watch_names")
        self.assertEqual(ast.unparse(invoke.func.body), "selected")
        self.assertEqual(ast.unparse(invoke.func.orelse), "scope[request['entry']]")
